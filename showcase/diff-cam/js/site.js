$(function () {
  ...
  function upload(capture) {
    return new Promise(function (resolve, reject) {
      $.ajax({
        type: 'POST',
        url: 'https://192.168.2.150:9091/upload',
        dataType: 'JSON',
        data: {
          key: 'Serifx',
          score: capture.score,
          time: new Date().toLocaleString().toLowerCase(),
          dataURL: capture.getURL()//.replace('data:image/png;base64,', '')
        },
        success: function (res) {
          $history.append($(`<p>${new Date().toLocaleString()} 上传成功</p>`));
          return resolve(res);
        },
        error: function (xhr, status, err) {
          $history.append($(`<p style="color: red;">${new Date().toLocaleString()} 上传失败 ${err.message || ''}</p>`));
          return reject(err);
        }
      });
    });
